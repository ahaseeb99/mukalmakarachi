        ...
        <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
          <div class="d-flex align-items-center flex-wrap mr-2">
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item active"><a href="{{route('admin.books.index')}}" class="text-muted">Books</a></li>
            </ul>
          </div>
                  ...
                  <tbody>
                    @foreach ($modules as $key => $item)             
                        <tr class="odd gradeX">
                              ...
                              <td class="2 text-center" ><img width="50px" src="{{asset($item->thumbnail)}}" /></td>
                              <td class="urdu 3 text-left"> {{$item->title}}</td>
                              <td class="6 text-left">{{number_format($item->price,2)}} Rs</td>
                              <td class="6 text-left">$ {{number_format($item->pdf_price,2)}}</td>
                              <td class="12 text-center">{{$item->category->title}}</td>
                              <td class="9 text-center">{{$item->author->title}}</td>
                              <td class="16 text-center">
                                <a href="{{route('admin.books.edit', $item->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit">
                                      ...
                                      <div class="modal-dialog modal-dialog-centered " role="document">
                                        <div class="modal-content">
                                          ...
                                          <div class="modal-footer">
                                            ...
                                            <a href="{{route('admin.books.delete', $item->id)}}"  class="btn btn-primary">Delete</a>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
