...
@section('css')
  ...
  <style>
    ...
    <link rel="stylesheet" href="{{asset('admin/assets/plugins/datatable/buttons.css')}}">
@endsection
          ...
          <div class="container">
            <div class="card card-custom">
              <div class="card-header flex-wrap py-5">
                <div class="card-title"><h3 class="card-label">{{$module->plural}}</h3></div>
                  ...
                  <tbody>
                   @foreach ($posts as $key => $item)
                        <tr class="odd gradeX">
                              ...
                              <td class="1 text-center" >#{{$item->id}}</td>
                              <td class="2 text-center" ><img width="50px" src="{{asset($item->thumbnail)}}" /></td>
                              <td class="3 text-left">{{$item->title}}</td>
                              <td class="7 text-center">{{$item->status}}</td>
                              <td class="9 text-center">{{$item->created_at->format('d-m-Y')}}</td>
                              <td class="16 text-center">
                                <a href="{{route('admin.posts.edit', $item->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="text-success la la-edit"></i></a>
                                <a href="{{route('admin.posts.delete', $item->id)}}" class="btn btn-sm btn-clean btn-icon" title="Delete"><i class="text-danger la la-trash"></i></a>
                              </td>
                        </tr>
                      @endforeach
                  </tbody>
